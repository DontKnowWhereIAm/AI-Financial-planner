...
def extract_tuition(rec: Dict) -> Dict[str, Optional[float]]:
...
def print_record(rec: Dict) -> None:
    ...
    tu = extract_tuition(rec)

    print("\n=== Match ===")
    print(f"Name     : {nm}")
    if city or st or zip_code:
        print(f"Location : {city}, {st} {zip_code}".strip().rstrip(", "))
    if url:
        print(f"URL      : {url}")
    print(f"{YEAR}.cost.tuition.in_state     : {tu['in_state']}")
    print(f"{YEAR}.cost.tuition.out_of_state : {tu['out_of_state']}")
...
def main():
    ap = argparse.ArgumentParser(description=f"Fetch {YEAR} in-state and out-of-state tuition for a U.S. college.")
